...
function buildHealthContext(useHealth, healthMetrics) {
  if (!useHealth) return '';
  ...
  const summary = summarizeHealthData(health.raw, healthMetrics);
...
async function generateAndSend(profile, overridePrompt) {
  ...
  if (overridePrompt) {
    ...
    healthCtx = buildHealthContext(profile.use_health_data, profile.health_metrics);
  } else {
    ...
    healthCtx = category === 'health' ? buildHealthContext(profile.use_health_data, profile.health_metrics) : '';
  }
